      ...
      <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-[15px]" @click.self="modalStore.closeStore()">
        <div class="solid-panel w-full max-w-lg md:max-w-xl relative overflow-hidden h-auto m-4 pointer-events-auto shadow-2xl">
            ...
            <section>
              ...
              <div class="flex flex-col gap-2.5">
                <button v-if="invitedCount === 0" @click="modalStore.closeStore(); handleInviteFriend()" class="group relative flex items-center justify-between px-4 py-5 rounded-2xl bg-white/[0.04] hover:bg-white/[0.08] active:scale-[0.96] focus:outline-none select-none transition-all duration-300 overflow-hidden w-full text-left">
                  ...
                  <div class="relative z-10 shrink-0 h-9 w-[110px] flex items-center justify-center rounded-full bg-white/[0.08] hover:bg-white/[0.12] transition-all duration-300 group-active:scale-95 text-white text-[13px] font-bold inter">
                    {{ $t('store.invite') }}
                  </div>
                </button>
               ...
               <div class="flex flex-col gap-3">
                  ...
                  <template v-else-if="starPackages.length > 0">
                    <div v-for="(pkg, index) in starPackages" :key="pkg.id" 
                         ...
                         class="group rounded-2xl p-4 flex items-center gap-4 transition-all duration-300 active:scale-[0.96] focus:outline-none select-none cursor-pointer min-h-[96px] box-border"
                       ...
                       <div class="shrink-0 h-9 w-[110px] flex items-center justify-center font-bold tracking-wide rounded-full transition-all duration-300 group-active:scale-95 ml-auto gap-1 text-black bg-white group-hover:bg-gray-50 shadow-sm">
                          {{ pkg.stars_count }}
                          <svg class="w-4 h-4 fill-current text-yellow-400" style="filter: drop-shadow(0px 0px 0.1px rgba(0,0,0,1));" viewBox="0 0 14 15">
          ...
          <div class="px-6 pb-7 pt-2 z-20">
            <button @click="modalStore.closeStore" class="w-full bg-white/[0.08] hover:bg-white/[0.12] active:scale-[0.96] focus:outline-none select-none transition-all duration-300 text-white font-bold rounded-xl py-3.5 text-[15px] inter">
              {{ $t('navigation.close') }}
            </button>
          </div>
        </div>
...
async function handleInviteFriend() {
  if (window.Telegram?.WebApp?.shareMessage) {
    try {
      const response = await api.get('/api/telegram/share-invite/');
